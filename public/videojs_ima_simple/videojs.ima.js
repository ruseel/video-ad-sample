...
(function(vjs) {
  ...
  imaPlugin = function(options, readyCallback) {
    ...
    player.ima.requestAds = function() {
      ...
      // forceNoneLinearFullSlot은 대체 뭐지??
      if (settings.forceNonLinearFullSlot) {
        adsRequest.forceNonLinearFullSlot = true;
      }


      // 아 nonLinearPlayerHeight은 1/3 height로 맞추는 구나...
     ...
     * videojs-ads-contrib.
     *   XXX videojs-ads-contrib를 위해 'adsready'를 날린다고?
     *   음... play는 자동으로 ima-sdk에서 알아서 하되 event는 날려주는 것 뿐일까?
     * @private
     */
    player.ima.onAdsManagerLoaded_ = function(adsManagerLoadedEvent) {
      ...
      if (!autoPlayAdBreaks) {
        // 왜 autoPlayAdBreaks가 false일때만 실행시키는 ㄲ가?
        try {
    ...
    player.ima.startFromReadyCallback = function() {
      // 자. 설명이 길구나.
      //  하면 vjs-ima를 쓸 때는 adsManager.start를 여기서 한 다는 거지?
      //    ima-sdk만 쓸때는 play버튼을 누르면
      if (autoPlayAdBreaks) {
        try {
          adsManager.init(
    ...
    player.ima.onAdBreakReady_ = function(adEvent) {
      // 어디서 등록했지? adBreakReadyListener를??
    ...
    player.ima.onContentPauseRequested_ = function(adEvent) {
      ...
      // 아 content가 pause 되었을 때 vjs-contrib-ads와 통신하는 것이 있구나...
      //  player.ads 와 관련된 부분만 싹 검색해보는 것도 일리가 있겠다.
      //  이거 intelij 에서는 지금 있는 곳이 좀 더 잘 보이려나?
     ...
     *  아... 광고를 위한 video 엘레먼트가 다 따로 있으니 이걸 따로 따로 해줘야 하나...
     * @private
     */
    ...
    // 뭐지이건 왜 _html5_api를 받은 것 뒤에 붙이지?
    // ??
    ...
    // 아. 여기서 vjs-contrib-ads 를 설정하는구나.
